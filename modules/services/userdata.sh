...
if [ -z "${proxy_endpoint}" ]; then
    ...
    echo "harvest_no_proxy=169.254.169.254,169.254.170.2,/var/run/docker.sock,${mlservices_endpoint}" >>/etc/graymeta/metafarm.env
    echo "no_proxy=localhost,127.0.0.1,169.254.169.254,s3.${region}.amazonaws.com,*.s3.${region}.amazonaws.com,$(echo ${elasticsearch_endpoint} | sed 's/https\?:\/\///'),${mlservices_endpoint}" >>/etc/graymeta/metafarm.env
